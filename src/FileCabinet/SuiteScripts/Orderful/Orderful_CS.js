...
define(['N/currentRecord'], function(currentRecord) {
    ...
    function pageInit(context) {
        // Keep legacy button styling for backward compatibility
    ...
    // New: processSelected will find the checked row in the inbox sublist and POST to the suitelet to start processing
    function processSelected() {
        try {
            // Find the checkbox that starts with name "custpage_select"
            var checks = document.querySelectorAll('input[name^="custpage_select"]');
            var selectedIndex = -1;
            for (var i = 0; i < checks.length; i++) {
                if (checks[i].checked) { selectedIndex = i; break; }
            }
            if (selectedIndex === -1) { alert('Please select one transaction to process.'); return; }
            // Attempt to find the corresponding transaction id input generated by NetSuite
            var transInput = document.querySelector('input[name="custpage_transid_' + selectedIndex + '"]') || document.querySelector('input[id*="custpage_transid" ][name*="_' + selectedIndex + '"]');
            var transId = transInput ? transInput.value : null;
            if (!transId) { alert('Could not determine transaction id for selected row.'); return; }

            if (!confirm('Start background processing for transaction ' + transId + '?')) return;

            var params = new URLSearchParams();
            params.set('action', 'process');
            params.set('custpage_selected_transid', transId);
            ...
            fetch(window.location.href, {
                ...
                try {
                    var display = document.getElementById('data-display');
                    if (!display) { alert('Processing started.'); return; }
                    if (text && text.indexOf('PROCESSING:') === 0) {
                        var parts = text.split(':');
                        var tid = parts[1] || '';
                        var mrid = parts[2] || '';
                        display.innerHTML = '<div id="orderful-processing-banner" style="padding:12px; border:1px solid #bee5eb; background:#d1ecf1; color:#0c5460; font-weight:600;">Processing started in the background.' + (tid ? ' Transaction: ' + tid : '') + '<br/><small id="orderful-processing-sub">Waiting for background task status...</small></div>';
                        if (mrid) pollTaskStatus(mrid);
                    } else {
                        display.innerHTML = '<div style="padding:12px; border:1px solid #ffeeba; background:#fff3cd; color:#856404;">' + (text || 'Processing started') + '</div>';
                    }
                } catch (e) { alert('Started processing.'); }
            }).catch(function(err) { alert('Error starting processing: ' + err.message); });
        } catch (e) { alert('Error: ' + e.message); }
    }

    function pollTaskStatus(mrId) {
        var attempts = 0;
        var maxAttempts = 60;
        var interval = setInterval(function() {
            attempts++;
            var p = new URLSearchParams(); p.set('action','taskStatus'); p.set('mrId', mrId);
            fetch(window.location.href, { method: 'POST', credentials: 'same-origin', headers: { 'Content-Type':'application/x-www-form-urlencoded' }, body: p.toString() }).then(function(res){ return res.json(); }).then(function(json){
                var sub = document.getElementById('orderful-processing-sub');
                if (!json) return;
                if (json.error) { if (sub) sub.innerText = 'Status error: ' + json.error; clearInterval(interval); return; }
                var status = json.status || '';
                if (sub) sub.innerText = 'Background task status: ' + status;
                if (status && status.toUpperCase() !== 'PENDING' && status.toUpperCase() !== 'PROCESSING') {
                    clearInterval(interval);
                    if (sub) sub.innerText += ' — finished';
                }
            }).catch(function(e){ if (attempts >= maxAttempts) clearInterval(interval); });
            if (attempts >= maxAttempts) clearInterval(interval);
        }, 5000);
    }

    function clearSelectedPoller() {
        try {
            var checks = document.querySelectorAll('input[name^="custpage_select"]');
            var selectedIndex = -1;
            for (var i = 0; i < checks.length; i++) { if (checks[i].checked) { selectedIndex = i; break; } }
            if (selectedIndex === -1) { alert('Please select one transaction to clear from poller.'); return; }
            var transInput = document.querySelector('input[name="custpage_transid_' + selectedIndex + '"]') || document.querySelector('input[id*="custpage_transid" ][name*="_' + selectedIndex + '"]');
            var transId = transInput ? transInput.value : null;
            if (!transId) { alert('Could not determine transaction id for selected row.'); return; }
            if (!confirm('Clear transaction ' + transId + ' from poller?')) return;
            var params2 = new URLSearchParams(); params2.set('action','clearPoller'); params2.set('transId', transId); var pollField = document.querySelector('input[name="custpage_poller_id"]'); if (pollField) params2.set('pollerId', pollField.value);
            fetch(window.location.href, { method:'POST', credentials:'same-origin', headers:{'Content-Type':'application/x-www-form-urlencoded'}, body: params2.toString() }).then(function(res){ return res.text(); }).then(function(text){ var display = document.getElementById('data-display'); if (display) display.innerHTML = '<div style="padding:12px; border:1px solid #0070d2; background:#e0f0ff;">' + text + '</div>'; else alert(text); }).catch(function(e){ alert('Error clearing poller: ' + e.message); });
        } catch (e) { alert('Error: ' + e.message); }
    }

    function refreshInbox() { window.location.reload(); }

    return { pageInit: pageInit, inspectData: inspectData, processTransaction: processTransaction, processDataAndDownload: processDataAndDownload, processSelected: processSelected, refreshInbox: refreshInbox };
});